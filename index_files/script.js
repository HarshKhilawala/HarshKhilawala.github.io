...
function designEndingThreeCells() {
    let bachelorTableOddRows = document.querySelectorAll(".bachelor-terms-list tbody > tr:nth-of-type(odd)");
    let numOfRows = bachelorTableOddRows.length;

    for (let i = 0; i < numOfRows; i += 1) {
        let desiredRow = bachelorTableOddRows[i];

        // If odd row turned out to be spanning-row => it has only 3 children (excluding first two cells)
        if (desiredRow.className === "spanning-row") {
            let thirdChild = desiredRow.children[0];
            let fourthChild = desiredRow.children[1];
            let fifthChild = desiredRow.children[2];
            thirdChild.style.backgroundColor = "rgb(237 238 242)";
            fourthChild.style.backgroundColor = "rgb(237 238 242)";
            fifthChild.style.backgroundColor = "rgb(237 238 242)";
        } else {
